package ru.practicum.kafka_sprint_2;

import jakarta.annotation.PostConstruct;
import lombok.extern.slf4j.Slf4j;
import org.apache.kafka.clients.admin.AdminClient;
import org.apache.kafka.clients.admin.AdminClientConfig;
import org.springframework.stereotype.Service;
import ru.practicum.kafka_sprint_2.config.KafkaStreamsConfiguration;

import java.util.*;

@Service
@Slf4j
public class AdminService {
    private final KafkaStreamsConfiguration kafkaConfiguration;
    private AdminClient adminClient;


    public AdminService(KafkaStreamsConfiguration kafkaConfiguration) {
        this.kafkaConfiguration = kafkaConfiguration;
    }

    @PostConstruct
    public void initAdminClient() {
        this.adminClient = AdminClient.create(getKafkaBrokerProperties());
    }

    private Properties getKafkaBrokerProperties() {
        Properties properties = new Properties();
        properties.put(AdminClientConfig.BOOTSTRAP_SERVERS_CONFIG, kafkaConfiguration.getKafkaBootStrapServer());
        return properties;
    }


    public boolean isTopicExists(String topicName) {
        try {
            return adminClient.listTopics().names().get().contains(topicName);
        } catch (Exception e) {
            throw new RuntimeException("An error occurred while checking for topic availability: " + e.getMessage());
        }
    }

    public void waitForTopics(List<String> topicsName, long searchIntervalMs) {
        Set<String> remainingTopics = new HashSet<>(topicsName);

        while (!remainingTopics.isEmpty()) {
            Iterator<String> iterator = remainingTopics.iterator();

            while (iterator.hasNext()) {
                String topicName = iterator.next();
                log.info(String.format("Осуществляется поиск топика: %s", topicName));

                if (isTopicExists(topicName)) {
                    log.info(String.format("Топик: %s был найден", topicName));
                    iterator.remove();
                } else {
                    log.info(String.format("Топик: %s не был найден. Следующая попытка поиска через %s мин...",
                        topicName,
                        searchIntervalMs / 1000 / 60));
                }
            }

            if (!remainingTopics.isEmpty()) {
                try {
                    Thread.sleep(searchIntervalMs);
                } catch (InterruptedException e) {
                    Thread.currentThread().interrupt();
                    throw new RuntimeException("Ожидание создания топика было прервано: " + e.getMessage());
                }
            }
        }
    }
}

